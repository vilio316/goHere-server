...
app.get(
  "/distance/:sourceLat/:sourceLong/:destLat/:destLong",
  async (req, res) => {
...
app.get("/geocoded_location/:lat/:long", async (req, res) => {
  const location_details = await fetch(
    `https://maps.googleapis.com/maps/api/geocode/json?latlng=${req.params.lat},${req.params.long}&key=${process.env.PLACES_API_KEY}`
  );
